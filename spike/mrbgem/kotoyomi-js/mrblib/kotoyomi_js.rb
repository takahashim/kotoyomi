# Kotoyomi::JS — high-level Ruby API around the C primitives in kotoyomi_js.c.
#
# Kotoyomi::JS::Value is defined in C as a BasicObject subclass with
...
#
# Example:
#   doc = Kotoyomi::JS.global[:document]
#   doc[:title] = "hello"
...
module Kotoyomi
  module JS
    ...
    class Value
      ...
      def [](key)
        Value.new(JS._get(handle, key.to_s))
      end

      def []=(key, value)
        # Keep `v` in a local so the temp handle isn't released by GC
        # before JS._set crosses the WASM boundary.
        v = JS.wrap(value)
      ...
      def call(method, *args, &block)
        ...
        # IMPORTANT: hold the wrapped Values in `wrapped` until JS._call
        # returns. Otherwise mruby's GC may collect them between
        # `.handle` extraction and the WASM call, releasing the JS handles
        # we just built.
        wrapped = args.map { |a| JS.wrap(a) }
        arg_handles = wrapped.map(&:handle)
        result = Value.new(JS._call(handle, method.to_s, arg_handles))
      ...
      #   button.on(:click) { |ev| ... }
      # Pass options via the third arg, e.g. JS.object(once: true).
