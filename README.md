...
別ターミナルで `make watch` を起動すると、`deck.md` を保存するたびに `viewer/slides.json` を
再生成し、開いているビューアがその場で再描画します(ページ全体のリロードなし・wasm 再起動なし・
表示中のスライド位置やテーマは維持)。

```bash
make serve     # ターミナル1: 配信(wsv, :8000)
make watch     # ターミナル2: deck.md を監視 → slides.json 再生成 + reload 通知
```
